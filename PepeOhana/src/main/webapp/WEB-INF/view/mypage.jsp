 ...
 *	ExeMyPage.javaで抽出したネコ情報とメッセージ情報を表示する
 *	セッション情報からユーザー情報を抽出し、表示する
 *	ユーザー情報の下部にある「編集する」ボタンをクリックすると、ユーザー情報編集機能へ遷移する
 *	「ねこ情報を登録する」ボタンをクリックするとネコ情報登録機能へ遷移する
 *	登録されているネコ情報の下部にある「編集・削除する」ボタンをクリックすると、ねこ情報編集・削除機能へ遷移する
 *	ネコ情報が登録されていない場合はテキストを表示する
 *	メッセージ一覧の受信メッセージ項目では、受信したメッセージがある場合は受信した内容と「ネコ詳細」ボタンと「返信する」ボタンをドロップダウンメニューで表示し、
 *	「ネコ詳細」ボタンをクリックすると、該当するネコの詳細表示画面へ遷移する
 *	「返信する」ボタンをクリックすると、メッセージ画面へ遷移する
 *	受信したメッセージが無い場合はテキストを表示する
 *	送信メッセージ項目では、ユーザーが送信したメッセージがある場合は送信した内容と「ネコ詳細」ボタンと「削除する」ボタンをドロップダウンメニューで表示し、
 *	「ネコ詳細」ボタンをクリックすると、該当するネコの詳細表示画面へ遷移する
 *	「削除する」ボタンをクリックすると、そのメッセージの削除処理を行う
 *	送信したメッセージがない場合はテキストを表示する
 *Author:櫻井
